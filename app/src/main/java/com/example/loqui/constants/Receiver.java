package com.example.loqui.constants;

public class Receiver {
    public static String OPEN_JITSI = "open_jitsi";
    public static String CLOSE_JITSI = "close_jitsi";

    public static String CLOSE_OUTGOING_CALL_ACTIVITY = "close_outgoing_call_activity";
    public static String CLOSE_INCOMING_CALL_ACTIVITY = "close_incoming_call_activity";

    public static String JOIN_OUTGOING_CALL_ACTIVITY = "close_outgoing_call_activity";
    public static String DECLINE = "decline";
}
